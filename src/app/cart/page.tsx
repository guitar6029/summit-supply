...
export default function ShoppingCartOverview() {
  const { shoppingCart, increaseQuantity, decreaseQuantity, removeItem, clearAllItems } = useCart();
  ...
  return (
    <div className="min-h-screen mt-[10rem] border-t-[1rem] border-amber-300 p-10 bg-[#1b5659]">
      ...
      {shoppingCart.length === 0 && (
        <div className="grid grid-cols-5 gap-4 p-4">
          <div className="col-span-1">
            <span className="hiking-font text-3xl">Your cart is empty</span>
          </div>
        </div>
      )}
        ...
        shoppingCart.map((item: CartItem, index: number) => {
          return (
            ...
            >
              ...
              <div className="col-span-1">
                <Trash2 onClick={() => removeItem(item.id, item.size) } size={35} className="group-hover:cursor-pointer " />
              </div>
            </div>
          );
        })}
